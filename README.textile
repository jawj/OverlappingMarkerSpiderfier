...
bc. markersThatWillSpiderfy()

Returns an array of all markers that will be spiderfied when clicked — i.e. that are within @nearbyDistance@ pixels of any other marker. Don't cache this — or if you do, be sure to invalidate the cache when the zoom level changes and when any marker is added, moved, hidden or removed.
