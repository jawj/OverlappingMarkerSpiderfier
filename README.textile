...
The Google Maps API code seems to change frequently, and lately has been breaking z-indices a lot. Also, the 'frozen' versions seem to be a lie: the 'frozen' 3.3 was working fine until recently, and then broke. At this moment, the 'stable' 3.5 seems to work well, but there are no guarantees.
...
The @options@ argument is an optional @Object@ with three available boolean properties.

h2. @markersWontMove@ and @markersWontHide@
...
However, if you know that you won't be moving and/or hiding any of the markers you add to this instance, you can save memory (a closure per marker in each case) by setting properties named @markersWontMove@ and/or @markersWontHide@ to @true@ (or anything "truthy":http://isolani.co.uk/blog/javascript/TruthyFalsyAndTypeCasting).
