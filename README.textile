...
Note: I recommend you use it with version 3.3 of the Maps API. This version is slower, but at the time of writing the &lt;canvas&gt;-based rendering in versions 3.4 and 3.5 appears to have broken handling of marker z-indices.
...
The Google Maps API &lt;script&gt; tag must be included before this code.
...
The @options@ argument is an optional @Object@ with two possible properties.
...
However, if you know that you won't be moving and/or hiding any of the markers you add to this instance, you can save memory (a closure per marker in each case) by setting properties named @markersWontMove@ and/or @markersWontHide@ to @true@.
...
In the case of @'click'@ events, @listenerFunc@ receives two arguments: first, the clicked marker object; second, an @Array@ of marker objects that were not clicked.

For @'spiderfy'@ or @'unspiderfy'@ events, @listenerFunc@ also receives two arguments: first, an @Array@ of the markers that were spiderfied or unspiderfied; second, an @Array@ of the markers that were not.
